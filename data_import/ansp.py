    ...
    for page, lines in pages:
        ...
        for line in lines:
            ...
            if re.match(r'^ *[=\-]', line):
                ...
                if current_name is not None and 'specimen_detail' not in current_name and current_label == 'name' and line.startswith(' '):
...
def split_fields(names: DataT) -> DataT:
    for name in names:
        ...
        if any('ANSP' in key for key in name):
            ...
            match = re.match(r'^ *(ANSP [^\.]+)\. ([^\.\(]*)(.*)$', name['specimen_detail'])
        ...
        if type_kind in ('holotype', 'lectotype'):
            ...
            if 'rest' in name:
                print(name['rest'])
                match = re.match(r'^\. ([^,]+), collected by (.*?) at (.*), (in )?([A-Z][a-z]+ \d+, \d{4})\.? *$', name['rest'])
                if match:
...
def main() -> DataT:
    ...
    config = lib.NameConfig(authority_fixes={
        'LeConte': 'Le Conte',
        'DuChaillu': 'Du Chaillu',
    }, original_name_fixes={
        'Sciurus fremontii': 'Sciurus Frémonti',
    })
    names = lib.associate_names(names, config, start_at='Onychomys ruidosae')
    lib.write_to_db(names, SOURCE, edit_if_no_holotype=False, always_edit=True, dry_run=False)
