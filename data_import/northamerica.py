...
def extract_taxa(pages: Iterable[Tuple[int, List[str]]]) -> DataT:
    ...
    def _assert_no_type_loc() -> None:
        assert not type_loc_lines, f'duplicate type loc while processing {current_taxon} / {line} / {type_loc_lines}'
        assert 'type_locality' not in current_taxon, f'duplicate type loc while processing {current_taxon} / {line}'

    def flush_type_loc() -> None:
        nonlocal type_loc_lines
        if type_loc_lines:
            assert 'type_locality' not in current_taxon, f'duplicate type loc while processing {current_taxon} / {type_loc_lines}'
    ...
    for current_page, lines in pages:
        ...
        for line in lines:
            ...
            if not line:
                pass
            elif re.match(r'^[a-z]+[ -—]+group$', line) or re.match(r'^(Order|Suborder|Superfamily|Family|Subfamily) [A-Z]{2,} ', line):
                flush_name()
                flush_type_loc()
                ...
                current_taxon = _make_taxon(line, 'genus', current_page)
                    ...
                    re.match(r'^[A-Z][a-z]+( [a-z]+){1,2} (von |de )?([A-Z]\. )*(Mc)?[A-Z][a-z]+(-[A-Z][a-z]+)?( and ([A-Z]\. )*[A-Z][a-z]+)?$', line) or
                    re.match(r'^[A-Z][a-z]+( [a-z]+){1,2} \(([A-Z]\. )*[A-Z][a-z]+(-[A-Z][a-z]+)?\)$', line)):
                flush_name()
                flush_type_loc()
                yield current_taxon
                ...
                match = re.match(r'^Typ(e|ical) [lL]ocalit ?y[,\. \-]+?— (?P<rest>.*)$', line)
                if match:
                    ...
                    if range_marker in line:
                        current_taxon['type_locality'] = [line[:line.index(range_marker)]]
                    else:
                        type_loc_lines = [line]
                elif type_loc_lines:
                    if range_marker in line:
...
def name_of_text(text: str, is_genus: bool) -> Dict[str, Any]:
    ...
    name: Dict[str, Any] = {key: value for key, value in match.groupdict().items() if value}
    name['is_genus'] = is_genus
    name['raw_text'] = text
    name_author = name['orig_name_author'].replace(' [sic]', '').replace(' (sic)', '').replace('[', '').replace(']', '')
...
def get_taxon_root_name(heading: str) -> Optional[str]:
    match = re.match(r'^([A-Z][a-z]+( [a-z]+){1,2}) (von |de )?(([A-Z]\. )*[A-Z][a-z]+|\([A-Za-z\. \-]{3,}\))', heading)
    if match:
    ...
    for taxon in taxa:
        ...
        if not is_genus:
            ...
            if not is_singleton:
                ...
                assert taxon_root_name is not None, f'failed to match {taxon["heading"]}'
            if taxon_root_name:
        ...
        if not is_genus and not is_singleton:
            if taxon_root_name not in seen_root_names:
                assert False, f'{taxon_root_name} is not in {seen_root_names} for {taxon}'
