...
def split_fields(names: DataT, refs_dict: RefsDictT) -> DataT:
    for name in names:
        ...
        if text:
            ...
            match = re.search(
                r"(preoccupied by|incorrect subsequent spelling( of)?(, but not)?|"
                r"unjustified emendation of|replacement name for) "
                r"([^;\d=]+?)(, \d{4}|;|$| \(preoccupied\)|, on the assumption)",
                text,
            )
            if match:
                name["variant_kind"] = {
            ...
            if text.startswith("type locality"):
                ...
                match = re.match(
                    r"type species (.*?), (\d{4}[a-z]?), ?by (.*)(;|$)", text
                )
                if match:
...
def main() -> None:
    ...
    config = lib.NameConfig(
        ...
        ignored_names={
            ...
            (
                "Peramys brachyurus: Lesson, 1842:187 (= Didelphys dimidiata",
                "J. A. Wagner",
            ),
