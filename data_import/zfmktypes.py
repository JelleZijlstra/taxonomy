...
LABELS = {
    'Holotype', 'Paratypes', 'Current name', 'Comments', 'Paratype', 'Material', 'Lectotype', 'Neotype', 'Syntypes', 'Paralectotype',
    'Paralectotypes', 'Current status',
}
    ...
    for page, lines in pages:
        ...
        for line in lines:
            ...
            if re.match(r'^[A-Z][a-zA-Z]{4,}$', line):
                continue
            match = re.match(r'^(?P<original_name>[A-Z][a-z]+( \([A-Za-z][a-z]+\))?( [a-zü]+){1,2}) (?P<authority>[A-Z].*), (?P<year>\d{4})$', line)
            if match:
                if current_name:
                    assert 'Current name' in current_name or 'Current status' in current_name, f'missing comments for {current_name} on page {page}'
                    yield current_name
                current_name = {
                    'pages': [page],
                    **match.groupdict(),
    ...
    for name in names:
        ...
        if 'specimen_detail' in name and 'Syntypes' not in name:
            ...
            match = re.match(
                ...
                text)
            if not match:
                match = re.match(r'^(?P<type_specimen>ZFMK [\d\.]+)( \([^\)]+\))?, (?P<age_gender>[^,]+), (?P<body_parts>[^;]+); (?P<loc>.*), (collected|caught|coll\.) by .*$', text)
                if not match:
                    if verbose:
