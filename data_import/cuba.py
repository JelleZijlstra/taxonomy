    ...
    for page, lines in pages:
        for line in lines:
            ...
            if re.match(r'^\d\. ', line):
                ...
                continue
            if re.match(r'^\d\.\d\. ', line) or re.match(r'^[A-Z][a-z]+$', line) or not line:
                continue
            elif line.startswith('ESPECIE'):
                if current_name:
                    yield {'raw_text': current_name, 'pages': [current_page], 'collection_name': collection_of_name}
                current_name = [line]
                current_page = page
...
def split_fields(names: DataT) -> DataT:
    for name in names:
        ...
        if name['collection_name'] in COLLECTION_TO_LABEL:
            name['collection'] = models.Collection.by_label(COLLECTION_TO_LABEL[name['collection_name']])
...
def main() -> DataT:
    ...
    names = lib.associate_names(names, lib.NameConfig(authority_fixes={
