...
def realign_lines(pages: Iterable[Tuple[int, List[str]]]) -> Iterable[Tuple[int, List[str]]]:
    for page, lines in pages:
        initial_space = min((lib.initial_count(line, ' ') for line in lines if line and not line.isspace()), default=0)
...
def build_refs_dict(refs: DataT) -> RefsDictT:
    ...
    for ref in refs:
        text = ref['raw_text']
        match = re.match(r'^(.*?)[,\.]\'? \[?(\d+(–\d+)?[a-z]?)\]?( [\[\(]\d{4}[\]\)])?\.', text)
        ...
        if ' and ' not in raw_authors:
            ...
            if authors in ('Allen', 'Davis', 'Geoffroy St.-Hilaire', 'Fischer', 'Gervais', 'Peters', 'Nelson', 'Howell',
                           'LeConte', 'Miranda-Ribeiro', 'Lima', 'Smith', 'Anderson', 'Vieira', 'Peterson', 'Johnson',
                           'Shaw', 'Peale', 'Owen', 'Cuvier', 'Carter', 'Brown', 'Contreras', 'Freitas', 'Gardner'):
...
def split_text(names: DataT) -> DataT:
    for name in names:
        ...
        match = re.match(r'^(?P<name_authority>[^\d]+?),? (?P<year>\d{4}[a-z]?): ?(?P<page_described>[^;]+?)(, (?=type locality )|; )(?P<rest>.*)$', name['raw_text'])
        if not match:
...
def split_fields(names: DataT, refs_dict: RefsDictT) -> DataT:
    for name in names:
        ...
        if text:
            ...
            match = re.search(r'(preoccupied by|incorrect subsequent spelling or invalid emendation of|incorrect subsequent spelling( of)?(, but not)?|'
                              r'unjustified emendation of|replacement name for|lapsus calami for) '
                              r'([^;\d=]+?)(, \d{4}|;|$| \(preoccupied\)|, on the assumption|\(but to be|\(see Remarks|\[preoccupied by|\(preoccupied by)', text)
            if match:
                name['variant_kind'] = {
                ...
                name_authority = split_name_authority(match.group(4), quiet=True, try_harder=True)
                if name_authority:
            ...
            if text.startswith('type locality'):
                ...
                match = re.match(r'type species (?P<type_name>.*?)( \([^\)]+\))?(, (?P<type_year>\d{4}[a-z]?))?, ?by (?P<type_kind>.*)(;|$)', text)
                if match:
...
def main() -> None:
    ...
    names = lib.associate_names(names, config, max_distance=2, try_manual=True, start_at='Sciurus granatensis agricolae')
