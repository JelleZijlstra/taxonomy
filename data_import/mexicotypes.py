...
def associate_names(names: DataT) -> DataT:
    ...
    for name in names:
        name["raw_name"] = {
            k: v for k, v in name.items() if k not in ("text", "name_author")
        }
        name_obj = None
        total += 1
        author = (
            ...
            .replace("G. M.", "G.M.")
            .replace("J. A.", "J.A.")
            .replace("Tshudi", "Tschudi")
            .replace("Bachmann", "Bachman")
        )
...
def write_to_db(names: DataT, dry_run: bool = True) -> None:
    ...
    for name in names:
        ...
        if "type_locality" in name:
            ...
            if nam.type_locality != name["type_locality"]:
                if nam.type_locality is not None:
                    print(
                        f'value for type locality differs: (new) {name["type_locality"]} vs. (current) {nam.type_locality}'
                    )
                    print(name["loc"])
                    print(name["state"])
                    print(nam.type_tags)
        ...
        if (
            ...
            and nam.original_name != name["scientific_name"]
        ):
            num_changed["discrepancies"] += 1
            new_root_name = helpers.root_name_of_name(
                name["scientific_name"], constants.Rank.species
            )
            if (
                ...
                name_discrepancies.append(
                    (nam, nam.original_name, name["scientific_name"])
                )

        if not dry_run:
            nam.add_comment(
                constants.CommentKind.structured_quote,
                json.dumps(name["raw_name"]),
                SOURCE,
                pages,
            )
            nam.save()
