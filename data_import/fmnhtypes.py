    ...
    for page, lines in pages:
        ...
        for line in lines:
            ...
            if re.match(r'\s+(Order|Family) +[A-Z]+$', line):
                continue
            match = re.match(r'^(?P<original_name>[A-Z][a-z]+( +\([A-Z][a-z]+\))?( +[a-z\-]+(\[[a-z]+\])?){1,2}) +'
                             r'(?P<authority>([A-Z]\. +)*[A-Z][a-z]+( +and +[A-Z][a-z]+)?) *(?P<type_kind>Cotypes|Lectotype)?$', line)
            if match:
                if current_name:
                    assert 'specimen_detail' in current_name, f'missing specimen for {current_name} on page {page}'
                    yield current_name
                current_name = {
                    'pages': [page],
                    'raw_text': [line],
                ...
                assert 'specimen_detail' not in current_name, f'{line}: already saw specimen detail for {current_name} at {page}'
                start_label('synonymy', line)
    yield current_name
...
def split_fields(names: DataT) -> DataT:
    for name in names:
        ...
        match = re.match(r'^(?P<specimen>\d+)\. +(?P<body_parts>[^\.]+)\. +(?P<age_gender>[^\.]+)\. +'
                         r'(?P<loc>.*?)\. +(Altitude +(?P<altitude>[\d,\-]+ +(feet|meters))\. +)?((?P<date>([A-Z][a-z]+( \d+)?, )?\d{4})\. +)?'
                         r'(Received from |Presented by |Collected by|Purchased |Found by)', text)
        if match:
