...
LABELS = ["Holotype", "Remarks", "Type Locality", "Type locality"]
...
def extract_pages(lines: Iterable[str]) -> Iterable[tuple[int, list[str]]]:
    ...
    for line in lines:
        ...
        if match:
            ...
            match = re.search(
                r"TYPE SPECIMENS Of MAMMALS +(\d+) *$", line, re.IGNORECASE
            )
            if match:
                if current_page is not None:
                    yield current_page, current_lines
            ...
            if not is_blank and not is_header:
                ...
                if match:
                    ...
                    if last_line_header and last_line_blank:
                        ...
                        is_new_name = line.lstrip()[0].isupper() and bool(
