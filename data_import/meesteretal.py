...
def split_fields(names: DataT) -> DataT:
    for name in names:
        ...
        if '$' in text:
            ...
            match = re.match(r'^(?P<year>\d{4})\.\s+(?P<orig_name_author>[^,]+(, +var\.[^,]+|(?<=[A-Z]), [^,]+)?), '
                             r'(?P<verbatim_citation>.*)$', head.strip())
            if match:
            ...
            match = re.match(r'^(?P<year>\d{4})\.\s+(?P<orig_name_author>[^,]+(, +var\.[^,]+|(?<=[A-Z]), [^,]+)?(, Morrison-Scott & Hayman)?), '
                             r'(?P<rest>.*)$', text)
            if match:
                name['year'] = match.group('year')
                name['orig_name_author'] = match.group('orig_name_author')
                rest = match.group('rest')
                match = re.match(r'(?P<verbatim_citation>.+?(: ?[,\d IBgo\-]+| (fig|pl)\. [I\d]+| and text|, footnote))'
                                 r'\. (?P<rest>.*)$', rest)
                if match:
                    ...
                    match = re.match(r'^(?P<verbatim_citation>.*\d( \([^\)]+\))?)\. (?P<rest>.*)$', rest)
                    if match:
                        name.update(match.groupdict())
                    else:
                        print(f'failed to match {text}')
