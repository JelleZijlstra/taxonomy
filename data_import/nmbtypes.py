...
def split_fields(names: DataT) -> DataT:
    for name in names:
        ...
        type_kind = name['type_specimens'].split()[0].replace('[', '').replace(']', '').lower()
        ...
        if type_kind in ('holotype', 'lectotype'):
            ...
            match = re.match(r'^[^ ]+ (?P<type_specimen>NMB ?[\dM\-]+)( \([^\)]+\)){0,2} 1(?P<gender>[MF\?]) (?P<body_parts>[^:]+): (?P<loc>.*)$', text)
            if match:
                name.update(match.groupdict())
            else:
                print(f'failed to match {name["type_specimens"]}')
        name['specimen_detail'] = lib.clean_string(name['type_specimens'].replace(MARKER, ' '))
        yield name
...
def main() -> DataT:
    ...
    names = lib.associate_names(names, name_config=lib.NameConfig(authority_fixes={'Heim De Balsac': 'Heim de Balsac'}), start_at='Mus xanthurus orientalis')
