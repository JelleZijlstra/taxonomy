...
def extract_names() -> Iterable[Dict[str, Any]]:
    ...
    for page, lines in align_columns():
        ...
        for line in lines:
            ...
            if re.match(r'^\s*(Genus|Family|Subfamily|Order) [A-Z]+ [A-Z][a-zA-Z ]+$', line):
                continue
            # ignore blank lines
            if not line:
                continue
            ...
            elif current_label in ('CONDITION', 'COMMENT', 'COLLECTOR') or current_label is None:
                # new name
                if current_name is not None:
                    yield current_name
    ...
    for name in clean_text():
        ...
        for type_key in ('HOLOTYPE', 'LECTOTYPE', 'NEOTYPE'):
            if type_key in name:
                ...
                name['species_type_kind'] = getattr(constants.SpeciesGroupType, type_key.lower())
...
def associate_names() -> Iterable[Dict[str, Any]]:
    ...
    for name in extract_data():
        name_obj = None
        total += 1
        for original_name, authority in name_variants(name['original_name'], name['authority']):
                            ...
                            if not dry_run:
                                ...
                                if getinput.yes_no(f'Is the original spelling {nam.original_name} correct? '):
                                    nam.add_variant(new_root_name, constants.NomenclatureStatus.incorrect_subsequent_spelling,
                                                    paper=SOURCE, page_described=name['pages'][0], original_name=new_value)
                                    continue
                        else:
