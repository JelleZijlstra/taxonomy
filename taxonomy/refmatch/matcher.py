...
CLEAR_CROSSREF_CACHE_EVERY = 500
...
STRICT_PAGE_TYPES = {"journal_article", "book_chapter", "report", "thesis"}
STOP_TITLE_WORDS = {
    "a",
    "an",
    "and",
...
class ReferenceDatabase(Protocol):
    ...
    def get_citation_group(self, citation_group_id: int) -> CitationGroup:
        raise NotImplementedError
...
class TaxonomyDatabase:
    ...
    def get_article_citation(self, article_id: int) -> str:
        return article_citation(get_article(article_id))

    def original_citation_ids_for_taxon(self, label: str) -> tuple[int, ...]:
        return original_citation_ids_for_taxon(label)

    def get_citation_group(self, citation_group_id: int) -> CitationGroup:
        return get_citation_group(citation_group_id)
...
def build_article_index() -> ArticleIndex:
    ...
    return build_article_index_from_records(records)


def build_article_index_from_records(records: Iterable[ArticleRecord]) -> ArticleIndex:
...
def find_candidates(
    ...
    if taxon_groups:
        for page_text, labels in taxon_groups:
            for label in labels:
                for article_id in database.original_citation_ids_for_taxon(label):
...
def score_volume_pages(
    ...
    if row_start and article.start_page:
        if row_start == article.start_page:
            ...
            if row["reference_type"] in STRICT_PAGE_TYPES:
                score -= 18
                reasons.append("start page conflicts")
            else:
                score -= 5
                reasons.append("start page differs")
        elif row_page_in_article_range(row, article):
            score += 6
    ...
    elif row_end and article.end_page and row["reference_type"] in STRICT_PAGE_TYPES:
        score -= 8
        reasons.append("end page conflicts")
    return score, reasons
...
def row_page_in_article_range(row: dict[str, str], article: ArticleRecord) -> bool:
    row_page = page_number(first_page(row["pages"]))
...
def has_strict_page_conflict(row: dict[str, str], article: ArticleRecord) -> bool:
    """Return whether page evidence argues against an automatic match.

    Books often have edition-, chapter-, or taxon-specific page references, so
    strict page disagreement is only treated as blocking evidence for reference
    types where the parsed pages should identify the same article-like unit.
    """
    if row["reference_type"] not in STRICT_PAGE_TYPES:
        return False
    row_start = first_page(row["pages"])
    row_end = last_page(row["pages"])
    if row_start and article.start_page and row_start != article.start_page:
        return True
    return bool(row_end and article.end_page and row_end != article.end_page)
...
def classify_match(
    ...
    if has_strict_page_conflict(row, best.article) and "url" not in best.methods:
        if best.score >= 72:
...
def infer_row_bhl_url(
    ...
    for citation_group_id in row_bhl_citation_group_ids(row, index, learned_mappings):
        try:
            citation_group = database.get_citation_group(citation_group_id)
        except Exception:  # noqa: S112
            continue
...
def citation_group_from_index(
    index: ArticleIndex, database: ReferenceDatabase, citation_group_id: int
) -> CitationGroup | None:
    article_id = index.citation_group_sample_article_id.get(citation_group_id)
    if article_id is None:
        return None
    return database.get_article(article_id).citation_group
...
def citation_group_is_doi_likely(
    ...
    if row_year is None:
        return False
    citation_group = citation_group_from_index(index, database, citation_group_id)
    if citation_group is None:
        return False
...
def row_doi_candidates_from_title_search(
    ...
    *,
    ...
    if not title or row_year is None:
        return ()
    if (
        not citation_group_is_doi_likely(citation_group_id, row_year, index, database)
        and row_year < 2000
    ):
        return ()
    citation_group = citation_group_from_index(index, database, citation_group_id)
    ...
    *,
    ...
    for citation_group_id in citation_group_ids:
        ...
        for doi in row_doi_candidates_from_title_search(
            row, citation_group_id, index, database, mode=mode
        ):
            if doi in seen:
                continue
            seen.add(doi)
...
def clear_crossref_memory_caches(processed: int, every: int) -> None:
    if every <= 0 or processed % every != 0:
        return
    api_data.clear_crossref_memory_caches()
    print(f"  Cleared in-memory CrossRef caches after {processed} rows.", flush=True)
...
def evaluate_row(
    ...
    *,
    ...
    candidates = find_candidates(row, index, database, learned_mappings)
    scored = sorted(
...
def run_matching_pass(
    ...
    *,
    ...
    clear_crossref_cache_every: int = CLEAR_CROSSREF_CACHE_EVERY,
    label: str,
    ...
    for row_number, row in enumerate(rows, start=1):
        ...
        if doi_mode != "off":
            clear_crossref_memory_caches(row_number, clear_crossref_cache_every)
...
def build_learned_mappings(
    evaluations: Sequence[RowEvaluation],
    index: ArticleIndex,
    *,
    doi_mode: str,
    clear_crossref_cache_every: int = CLEAR_CROSSREF_CACHE_EVERY,
    ...
    for row_number, evaluation in enumerate(evaluations, start=1):
...
def print_round_gains(
    ...
    for first, final_row in zip(first_pass, final_rows, strict=True):
        ...
        if final_row["bhl_url"] and not first.output_row["bhl_url"]:
            bhl_new_links += 1
    print(
        f"Final pass additions over learning pass: taxonomy_new_matches={taxonomy_new_matches} doi_new_links={doi_new_links} batlit_new_links={batlit_new_links} bhl_new_links={bhl_new_links}",
        flush=True,
    )
...
def external_columns(
    ...
    *,
    ...
    article = (
        database.get_article(match.article.id)
    ...
    if include_slow_links and not out["bhl_url"] and bhl_mode != "off":
        if bhl_url := infer_row_bhl_url(
            row, index, database, learned_mappings, bhl_mode
        ):
            out["bhl_url"] = bhl_url
    ...
    if not out["doi"] and doi_mode != "off":
        out["doi"], out["doi_source"] = row_doi_from_crossref(
            row, index, database, mode=doi_mode, learned_mappings=learned_mappings
        )
        doi = out["doi"]
    ...
    out.update(
        {
            ...
            "taxonomy_citation": database.get_article_citation(matched_record.id),
        }
    )
    return out
...
def run_match_csv(
    ...
    *,
    doi_mode: str = "cached",
    bhl_mode: str = "cached",
    doi_learning: bool = False,
    clear_crossref_cache_every: int = CLEAR_CROSSREF_CACHE_EVERY,
    database: ReferenceDatabase | None = None,
    index: ArticleIndex | None = None,
    build_index: Callable[[], ArticleIndex] = build_article_index,
    include_slow_links: bool = True,
) -> None:
    if database is None:
        database = TaxonomyDatabase()
    print(
        f"Reading parsed references from {input_path} (doi_mode={doi_mode}, bhl_mode={bhl_mode})...",
        flush=True,
    )
    ...
    learning_doi_mode = doi_mode if doi_learning else "off"
    first_pass, _first_summary = run_matching_pass(
        rows,
        index,
        database,
        doi_mode=learning_doi_mode,
        bhl_mode=bhl_mode,
        include_slow_links=False,
        clear_crossref_cache_every=clear_crossref_cache_every,
        label="Round 1 (learning)",
    )
    learned_mappings = build_learned_mappings(
        first_pass,
        index,
        doi_mode=learning_doi_mode,
        clear_crossref_cache_every=clear_crossref_cache_every,
    ...
    with output_path.open("w", newline="") as f:
        ...
        for row in final_rows:
            writer.writerow(row)
    print(f"Wrote taxonomy matches for {len(rows)} references to {output_path}")
