...
class _SuffixTree(object):
    ...
    def display(self, max_depth: int = 1, depth: int = 0, most_common: Optional[int] = None,
        ...
        for key, subtree in subtrees:
            ...
            subtree.display(max_depth=max_depth - 1, depth=depth + 1, most_common=most_common, min_count=min_count)
...
def genus_suffix_tree(no_complex_only: bool = False) -> _SuffixTree:
    query = models.Name.select(models.Name.root_name).where(models.Name.group == constants.Group.genus)
    if no_complex_only:
