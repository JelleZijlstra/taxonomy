...
matcher = importlib.import_module("taxonomy.refmatch.matcher")

OUTPUT_DIR = Path(__file__).resolve().parent / "output"
DEFAULT_INPUT = OUTPUT_DIR / "msw3-refs-parsed.csv"
DEFAULT_OUTPUT = OUTPUT_DIR / "msw3-refs-taxonomy-matches.csv"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Match parsed MSW3 references to taxonomy database Articles and external "
            "identifiers."
        )
    )
